...
function darksky(key, lat, lng, time) {
  ...
  var response = JSON.parse(
    UrlFetchApp.fetch(url, { muteHttpExceptions: false })
  );
...
function prompt() {
  var startRow = SpreadsheetApp.getActive()
    .getRangeByName("pStartRow")
    .getValue();
  run(startRow);
}
...
function run(startRow) {
  ...
  var address = SpreadsheetApp.getActive()
    .getRangeByName("pAddress")
    .getValue();
