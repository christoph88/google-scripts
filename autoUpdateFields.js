function autoUpdateFields(params, triggerField, valueField, updateValue) {
  ...
  var triggerCol = params.headers[0].indexOf(triggerField);
  ...
  if (valueCol > -1 && params.index > 1 && params.editColumn == triggerCol) {
    ...
    var checkIfModifiedDt =
      params.headers[0][params.headers[0].indexOf(valueField)] == "ModifiedDt";
...
function onEdit(event) {
  ...
  autoUpdateFields(
    ...
    "Task",
    "Priority",
    "=(if(isblank(F%);5;if(F%=0;1;F22))+if(isblank(E%);5;if(E%=0;1;E%)))*if(B%=0;1;B%+1)"
  );
